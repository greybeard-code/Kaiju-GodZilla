...
namespace NinjaTrader.NinjaScript.Indicators.GreyBeard.KingPanaZilla
{
	...
	public class gbNobleCloud : Indicator
	{
		...
		protected override void OnBarUpdate()
		{
			try
			{
				...
				if (this.CloudEnabled && this.CloudOpacity > 0 && base.CurrentBar >= base.BarsRequiredToPlot && this.cloudState != 0)
				{
					...
					// Defense #4: evict old cloud-region tags whose cloudIndex is
					// > DRAW_TAG_KEEP bars in the past. Each cloud cycle creates
					// a new tag keyed on its starting bar; without eviction these
					// accumulate across cloud transitions over a long session.
		...
		private void PrintMarker(bool isBullish)
		{
			...
			// Defense #4: evict the marker tag from DRAW_TAG_KEEP bars ago. Cheap
			// no-op when tag doesn't exist; bounds the chart's draw-object pool.
		...
		// Defense #4: rolling drawing-tag cleanup. Each new draw evicts the
		// corresponding tag from DRAW_TAG_KEEP bars ago. Without this,
		// "gbNobleCloud.marker.{CurrentBar}" tags accumulate forever (per-bar
		// uniqueness on Draw.Text) and "gbNobleCloud.cloud.{bullish|bearish}.{cloudIndex}"
		// tags accumulate per cloud cycle. Over a multi-hour session the chart's
		// draw-object pool exhausts WPF resources → "Not Enough Quota" lockup.
		// Per feedback_nt8_wpf_quota_prevention.md and GODZILLA_LOCKUP_PREVENTION.md.
