...
from config import ACCT_LABEL, ACCT_GRADE, HUB_FILE
from log_parser import compute_stats, group_by_session
...
def build_hub_html(all_trades: List[dict], index: dict,
  ...
  <div>
    ...
    <div class="meta">MNQ 06-26 &nbsp;·&nbsp; APEX …084 (G4) + APEX …085 (G3) &nbsp;·&nbsp; Automated Surveillance Active</div>
  </div>
</div>"""
  ...
  {kpi_card("All-Time P&L",  pnl_fmt(c.get('pnl',0)),        pnl_class(c.get('pnl',0)),   f"{c.get('trades',0)} live trades")}
  ...
  {kpi_card("084 (G4)",      pnl_fmt(c.get('pnl_084',0)),     pnl_class(c.get('pnl_084',0)), f"{c.get('trades_084',0)} trades")}
  {kpi_card("085 (G3)",      pnl_fmt(c.get('pnl_085',0)),     pnl_class(c.get('pnl_085',0)), f"{c.get('trades_085',0)} trades")}
</div>"""
  ...
  <div class="rec-body">{r.get('body','')}</div>
    ...
    live_days = sorted(
        [d for d in by_day if any(t['is_live'] for t in by_day[d])],
        reverse=True
    )[:10]
    ...
    if live_days:
        session_rows = ''
        for d in live_days:
            day_trades = [t for t in by_day[d] if t['is_live']]
            s      = compute_stats(day_trades)
            fname  = f"reports/daily_{d.strftime('%Y%m%d')}.html"
            has_rpt= (reports_dir / f"daily_{d.strftime('%Y%m%d')}.html").exists()
...
<table><thead><tr>
  <th>Date</th><th>Accounts</th><th>Trades</th><th>W/L</th><th>P&L</th><th>Win %</th>
    ...
    # ── Footer ─────────────────────────────────────────────────────────────────
    footer = f'<div class="footer">Updated {now_stamp()} · MONARCH Intelligence Report System · MNQ 06-26</div>'
