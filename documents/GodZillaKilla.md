...
| Version | Summary |
|---|---|
| **1.10.1** | **Naked-position watchdog and ATM registration fixes.** No signal, entry, or exit logic changed. Four defects found after the strategy was silently disabled mid-session on two occasions (full analysis in the 2026-07-29 bug report). (1) `HasWorkingProtectiveOrders` rejected transitional order states, so a routine OCO quantity adjustment made a fully-bracketed position look naked — `ChangePending`, `ChangeSubmitted`, `CancelPending` and `PartFilled` now count as live protection. (2) The watchdog had no grace period after an entry fill; ATM brackets are not submitted atomically with the entry (351–644 ms measured), and the check could land inside that window — a fill, or a manual stop/target move, now suppresses the check for **Naked Watchdog Grace (sec)**. (3) The watchdog flattened on a single racy observation; the first sighting now logs `NAKED POSITION SUSPECTED` and only a second sighting **Naked Watchdog Re-Check (sec)** later logs `CONFIRMED` and flattens. (4) ATM position queries fired before `AtmStrategyCreate` had registered the ID, producing 6–12 NT8 log errors per entry — new `IsAtmIdQueryable()` gate with an **ATM Registration Quiet (sec)** fallback. Defense #8 received the same double-confirm treatment and its recovery was corrected: it called `FlattenEverything`, which cannot close an ATM-managed position and merely cancelled the brackets, leaving the position naked; it now closes at account level. Three new properties expose the timings, since all latency measurements came from the NT8 internal simulator. |
...
| **1.9.1** | **Performance & stability pass (no signal/entry behavior change).** Per-tick hot path trimmed: the playback/News-Filter connection check is cached (no reflection per tick), the ATM market-position lookup is fetched once per tick (staleness backstop and close-detection poll share it during open trades), and signal reads use a series-based `SafeSignalRead` overload that avoids a `Func<double>` closure allocation per read. Trade markers (`_markerList`) are now bounded — completed markers older than `DRAW_TAG_KEEP` bars are evicted with their draw objects, so the list no longer grows unbounded over long sessions. Removed a dead per-bar group-evaluation block, removed the vestigial always-true `_strategyEnabled` gate (master arm is `_autoArm`), and removed a cross-thread `BuildDashboardSnapshot()` call from the Auto-Arm button handler. Defense #8 forced-close PnL estimate now uses `Math.Round` instead of `RoundToTickSize`; martingale-close totals refresh inline instead of one tick late. |
| **1.9** | **Confirmation Bars** — new `Confirmation Bars` property (range 0–25, default 0). When set to N, the entry is held after the group trigger fires. On bar N, if price has moved in the signal direction (close higher than signal bar for long, lower for short), the entry submits. If not, the setup is dropped. Default 0 = immediate entry, identical to prior behavior. **Trade Marker** — new `Show Trade Marker` property (default on) draws a green `T` below the bar on long entries and a red `T` above on short entries, visible on both historical and realtime bars. **Enable Signal Tracking** moved from Signals to Dashboard Display section alongside the stats display properties it controls. |
...
| #8 | Mid-trade ATM ID goes stale (HDS bounce) | Double-confirms over 2s, then writes trade log, flattens at account level, resets all ATM state |
...
**Defense #8 hardening (v1.10.1):** Two problems, both observed in a single Playback run on 2026-07-13.

*It acted on one observation.* A single `GetAtmStrategyMarketPosition` hiccup five seconds after a healthy, callback-confirmed entry was enough to tear down all ATM state — the same weakness the naked watchdog had. Both legs now arm a suspect clock on the first sighting (`MID-TRADE ATM STALENESS SUSPECTED`) and only recover if the ATM still reports Flat `AtmStaleConfirmSeconds` later (`CONFIRMED`). The clock is cleared whenever the ATM recovers, when no matching trade is open, and on disable.

*Its recovery could not close the position.* Defense #8 called `FlattenEverything`, whose ATM branch runs `AtmStrategyClose` on the very ID NT8 has lost (a no-op) and `ExitLong`/`ExitShort` gated on `Position.MarketPosition` — always Flat in ATM mode, because the position belongs to the ATM rather than the strategy. The only statement that executed was `Account.Cancel` on the working orders. The result was the opposite of recovery: the brackets were cancelled and the position left open, and in the 2026-07-13 run this manufactured a naked 4-lot that the naked watchdog had to clean up twenty seconds later. Defense #8 now calls `FlattenNakedAccountPosition`, which closes at account level. This does disable the strategy — that was already the end state once the watchdog caught up, but it now happens immediately and without the unprotected window in between.
...
**Naked-position watchdog (rewritten v1.10.1):** `CheckForNakedPositions` runs on a 30-second wall-clock throttle and asks one question — the account holds a position, does it have working exit orders? If not, it flattens. Because that flatten goes through `Account.Flatten`, and NT8 disables every NinjaScript strategy on an account when the account is flattened, a false positive is not a nuisance — it is a silent halt of automated trading for the rest of the session. Eight such halts were logged across five days before v1.10.1. Three changes make the check trustworthy:

- **Transitional order states count as protection.** A healthy OCO bracket is not always `Working`/`Accepted`/`Submitted`. When one leg partially fills, NT8 issues a quantity change on the sibling, moving it through `ChangePending` and `ChangeSubmitted` while the filling leg sits in `PartFilled`. All four are now accepted. `CancelPending` is included because during an OCO transition the replacement is already in flight; the double-confirm covers the case where the cancel turns out to be final.
- **Grace period after order activity.** ATM brackets are not submitted atomically with the entry — 351 ms to 644 ms measured between fill and `Stop1`. Any fill on the instrument, and any manual stop/target move from the control-panel buttons, suppresses the check for **Naked Watchdog Grace (sec)**. Fills are stamped in two places because ATM fills do not reach `OnExecutionUpdate`; they arrive through the account-level execution hook.
- **Double confirmation.** The first sighting logs `NAKED POSITION SUSPECTED` and pulls the next check forward to **Naked Watchdog Re-Check (sec)**. Only a second consecutive sighting logs `NAKED POSITION CONFIRMED` and flattens. Worst-case cost is that a genuinely unprotected position stays exposed one re-check longer, which is well inside the 30-second polling granularity it already had.

The `SUSPECTED` line is also an early warning: it appears when the race window widens, before the window is wide enough to be destructive.

**ATM registration window (v1.10.1):** `atmStrategyId` is generated and stored *before* the asynchronous `AtmStrategyCreate` registers it inside NT8. Any position query in that window targets an ID NT8 does not know yet. NT8 does not throw for this — it logs at level 3 and returns `Flat` — so the existing `Try...Safe()` wrappers never caught it, and a fast market produced 6–12 identical errors per entry (11 within one millisecond in the worst observed case). `IsAtmIdQueryable()` gates every ATM position query until the create callback confirms registration, with an **ATM Registration Quiet (sec)** time fallback in case the callback never fires. Inside the window the helper returns `Flat`, which is what NT8 was returning anyway, minus the log flood. An ID belonging to a confirmed open trade is always queryable — necessary for the martingale leg, where the fill path sets `isAtmStrategyCreated` rather than `martingaleAtmStrategyCreated`.
...
|---|---|
| ATM Parameters | `OrderMode`, `AtmStrategy`, `MartingaleAtmStrategy`, `FixedOrderQuantity`, `FixedStopLossTicks`, `FixedProfitTargetTicks`, `AtmRegistrationQuietSeconds` |
| Signals | `ConfirmationBars`, `GroupTriggerSet1RequiredCount`, `UseKOSignals`…`UseNCSignals`, `RequireKOSignal`…`RequireNCSignal`, `KO_LongOperator`…`NC_ShortValue`, `G2_RequireKOSignal`…`G2_RequireNCSignal` |
...
| Risk | `EnableDailyProfitTarget`, `DailyProfitTarget`, `EnableDailyLossLimit`, `DailyLossLimit`, `NakedGraceSeconds`, `NakedReCheckSeconds` |
| Dashboard Display | `ShowDashboard`, `DashboardPosition`, `DashboardSize`, `EnableSignalTracking`, `ShowIndividualSignalStats`, `ShowGroupSignalTrackingStats`, `ShowControlPanel`, `ControlPanelPosition`, `ControlPanelLeft`, `ControlPanelTop`, `ControlPanelSize` |
| Indicator Display | `ShowTradeMarker`, `ShowBarStatusIndicator` |
