...
| Version | Summary |
|---|---|
| **1.4.1 Beta** | **Performance pass (no signal output change).** Child sub-indicators are now instantiated only when their signal is enabled in Set 1 or Set 2 — hosted indicators are calculated every bar whether or not they're read, so skipping unused ones avoids running heavy sub-indicators (ThunderZilla, SumoPullback, NobleCloud, etc.) for signals the user has turned off. The `OnBarUpdate` null-guard now only fails on a *needed* indicator. Signal reads use a series-based `SafeSignalRead` overload (no per-read closure allocation, null-safe for disabled indicators). HUD title now shows the indicator's instance name instead of a hardcoded literal. CSV log opens lazily on the first logged bar using `Time[0]` so Market Replay/Playback files are dated by session, not wall-clock. |
